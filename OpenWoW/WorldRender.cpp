...
void WorldRender::RenderPostprocess()
{
	...
	{
		float fogDist = m_WorldContoller->Map()->getSkyManager()->GetFog(LightFogs::LIGHT_FOG_DISTANCE);
...
void WorldRender::DSFogRenderPass()
{
	...
	_Render->getTechniquesMgr()->Postprocess_Fog->SetFogDistance(m_WorldContoller->Map()->getSkyManager()->GetFog(LightFogs::LIGHT_FOG_DISTANCE));
	_Render->getTechniquesMgr()->Postprocess_Fog->SetFogModifier(m_WorldContoller->Map()->getSkyManager()->GetFog(LightFogs::LIGHT_FOG_MULTIPLIER));
	_Render->getTechniquesMgr()->Postprocess_Fog->SetFogColor(m_WorldContoller->Map()->getSkyManager()->GetColor(LightColors::LIGHT_COLOR_FOG));
