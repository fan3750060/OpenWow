...
void ParticleSystem::update(float dt, uint32 _globalTime)
{
	float grav = gravity.getValue(manim, mtime, _globalTime);
	float deaccel = deacceleration.getValue(manim, mtime, _globalTime);

	// spawn new particles
	if (emitter)
	{
		float frate = rate.getValue(manim, mtime, _globalTime);
		float flife = 1.0f;
		flife = lifespan.getValue(manim, mtime, _globalTime);
		...
		else
		{
			...
			float w = areal.getValue(manim, mtime, _globalTime) * 0.5f;
			float l = areaw.getValue(manim, mtime, _globalTime) * 0.5f;
			float spd = speed.getValue(manim, mtime, _globalTime);
			float var = variation.getValue(manim, mtime, _globalTime);
			...
			if (en)
			{
				...
				{
					Particle p = emitter->newParticle(manim, mtime, w, l, spd, var, spr, spr2, _globalTime);
					// sanity check:
...
void ParticleSystem::setup(uint16 anim, uint32 time, uint32 _globalTime)
{
	manim = anim;
	mtime = time;
