...
void CWDL_LowResTile::Render3D()
{
	...
	_Render->TechniquesMgr()->WDL_LowRes_Pass->Bind();
	_Render->TechniquesMgr()->WDL_LowRes_Pass->SetShadowColor(/*vec3(1.0, 0.0, 0.0)*/_World->EnvM()->m_SkyManager->GetColor(LIGHT_COLOR_FOG));

	_Render->r.setGeometry(m_Geom);
	_Render->r.draw(PRIM_TRILIST, 0, 16 * 16 * 12);
	PERF_INC(PERF_MAP_LOWRESOLUTION);

	_Render->TechniquesMgr()->WDL_LowRes_Pass->Unbind();
