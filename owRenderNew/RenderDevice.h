...
public:
	...
	virtual std::shared_ptr<Scene> CreatePlane(float size, cvec3 N = vec3(0, 1, 0)) = 0;
	...
	virtual std::shared_ptr<Scene> CreateCylinder(float baseRadius, float apexRadius, float height, cvec3 axis = vec3(0, 1, 0)) = 0;
	...
	virtual std::shared_ptr<Scene> CreateArrow(cvec3 tail = vec3(0, 0, 0), cvec3 head = vec3(0, 0, 1), float radius = 0.05f) = 0;
