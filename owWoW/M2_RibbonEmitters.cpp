...
CM2_RibbonEmitters::CM2_RibbonEmitters(MDX* _model, IFile* f, const SM2_RibbonEmitter& _proto, cGlobalLoopSeq globals) :
	m_MDX(_model)
{
	m_Bone = &(m_MDX->GetBone(_proto.boneIndex));
...
void CM2_RibbonEmitters::draw()
{
	...
	// Geometry
	R_GeometryInfo* __geom = _Render->r.beginCreatingGeometry(_Render->Storage()->__layoutWater);
	...
	_Render->r.draw(PRIM_TRILIST, 0, vertices.size());
