...
public:
	...
	T getValue(uint32 anim, uint32 time)
	{
		...
		if (data[anim].size() > 1 && times[anim].size() > 1)
		{
			...
			for (uint32_t i = 0; i < times[anim].size() - 1; i++)
			{
	...
	void init(M2Track<D>& b, IFile* f, uint32* gs)
	{
		...
		// times
		for (uint32_t j = 0; j < b.timestamps.size; j++)
		{
			...
			for (uint32_t i = 0; i < pHeadTimes->size; i++)
			{
		...
		// keyframes
		for (uint32_t j = 0; j < b.nRanges; j++)
		{
			D* pHeadKeys = (D*)(f.GetData() + b.ofsKeys + j * sizeof(D));
